...
public class ResourcesUtil implements Serializable {
    ...
    public static Map<String, String> getKeyandValue(String baseName, Locale locale) {
        Map<String, String> map = new HashMap<>();
        ResourceBundle rb = ResourceBundle.getBundle(baseName, locale);
        Set<String> keyset = rb.keySet();
        for (Iterator<String> it = keyset.iterator(); it.hasNext();) {
            String key = (String)it.next();
            String value = ResourcesUtil.getValue(baseName, key);
            map.put(key, value);
        }
        return map;
    }
    ...
    /**
     * 通过key从资源文件读取内容，并格式化   public static Map<String, String> map = new HashMap<>();
     *
     * @param fileName
     *            资源文件名
     *
